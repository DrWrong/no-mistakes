...
// fail-closed contract: under the opt-out, only codex, claude, traex, and pi (whose
...
// TestNeutralizesGateInstructions_FalseWithoutOptOut proves codex, claude, traex, and pi
...
// closed for an unsupported harness and admits codex, claude, traex, and pi.
func TestEnsureGateNeutralized_RefusesUnsupportedUnderOptOut(t *testing.T) {
	...
	if err := EnsureGateNeutralized(optOutAgent(t, types.AgentTraex, nil)); err != nil {
		t.Errorf("traex must pass the gate under opt-out: %v", err)
	}
