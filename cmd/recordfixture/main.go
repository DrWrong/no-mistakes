...
//	go run ./cmd/recordfixture traex    --out internal/e2e/fixtures/traex
...
func run() int {
	...
	case "traex":
		return recordTraex(ctx, out, args)
	case "opencode":
		return recordOpencode(ctx, out, args)
	default:
		fmt.Fprintf(os.Stderr, "unknown agent %q (want claude|codex|traex|opencode)\n", agent)
		usage()
		return 2
	}
}
...
func usage() {
	fmt.Fprintln(os.Stderr, "usage: recordfixture <claude|codex|traex|opencode> --out <dir> [--bin <path>]")
