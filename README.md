...
- **Agent-agnostic** - `claude`, `codex`, `traex`, `rovodev`, `opencode`, `pi`, `copilot`, or `cursor` / `acp:<target>` via `acpx`, with ordered fallbacks; every gate requires a runnable configured pipeline agent.
...
`make e2e-record` overwrites `internal/e2e/fixtures/` from the real `claude`, `codex`, `traex`, and `opencode` CLIs, spends real API quota, and should be reviewed before committing.
