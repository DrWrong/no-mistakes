...
- **不挑 agent** —— 支持 `claude`、`codex`、`traex`、`rovodev`、`opencode`、`pi`、`copilot`，也可通过 `acpx` 使用 `cursor` / `acp:<target>`，并支持有序 fallback。
...
`make e2e-record` 会用真实的 `claude`、`codex`、`traex`、`opencode` CLI 覆盖 `internal/e2e/fixtures/`，会消耗真实 API 额度，提交前应当审查。
